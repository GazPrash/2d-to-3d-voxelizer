...
On Linux distributions, you must install **GTK3** and **WebKit2GTK** development libraries before building; (**Windows** and **MacOS** users can skip to the next step).
* **Debian / Ubuntu:**
  ```bash
  sudo apt update
...
Clone the main repo, navigate to the project directory, and run the following command to compile the binary:
```bash
...
### 5. Logging
We have an in built logger for debugging purposes and error tracking, browse to `backend/logger` and set:
```go
var DebugMode bool = true
```

## Upcoming (???) Features and Improvements
I currently have a lot of ideas regarding what's next for this app, top of my head here are the few things that i will be focusing on in the coming future;
- Provide an alternative option to the user to generate `.obj` and `.mtl` texture file seperately
- Add a minimal 3D editor in the viewer tab, so users can fix some in-accuracies generated by the coverter.

Feel free to request or suggest something in Issues.